...
class ModelConfig:
    """模型配置"""
    policy_type: str = "mlp"         # "mlp" | "vla" | "diffusion"
...
class AlgorithmConfig:
    """算法配置"""
    name: str = "bc"                 # "bc" | "sac" | "cql" | "recap"
    lr: float = 1e-4
    batch_size: int = 256
    gamma: float = 0.99
    tau: float = 0.005               # 软更新系数 (SAC)
    alpha: float = 0.2               # 温度系数 (SAC)
...
def load_config_from_yaml(config_path: str, config_name: str = "offline_bc") -> Config:
    ...
    algo_config = _dict_to_dataclass(cfg.get("algorithm", {}), AlgorithmConfig)
